...
def ValidacionDominios(GDB, RutaSalida):
    arcpy.env.workspace = GDB
    arcpy.CheckOutExtension("spatial")
    file = open(os.path.join(RutaSalida,'Error_Tablas.txt'), "w")
...
def estado(a):
    if (a>1):
        ...
        b = "OMITIR"
        return b"""
def GeneracionDeMarcos(GDB,RutaSalida, Escala, Limite_Del_Proyecto, Generacion_Marcos_Control):
    arcpy.env.overwriteOutput = True
    ##########################################################################################################
    ###Get parameters
    ###-------------------------------------------------------------------------------------------------------
    ...
    if script == 'true':
        ...
        if num_mr <= 15:
            ...
            filet.write("El numero de elementos categoria otros en todas las celdas es "+ str((numelementos*num_mr)/30) + "\n")
            filet.write("El numero de elementos categoria bacm en todas las celdas es "+ str((numelementos_bacm* num_mr)/30) + "\n")
...
def MarcosDeControlOrtoyMDT(Limite_Del_Proyecto,RutaSalida,Escala):
    ...
    return
def RevisionCurvasNivelyDrenajes(RutaSalida):
    # Script arguments
    Ruta_GDB_Salida = RutaSalida
    Feature_Class_Curvas_Nivel = os.path.join(str(GDB),'Elevacion\CNivel')
    Feature_Class_Drenaje_L = os.path.join(str(GDB),'Hidrografia\DAgua_P')
    # Local variables:
    Salida_gdb = os.path.join(Ruta_GDB_Salida, "Revision_Curvas.gdb")                                            #ok
    ...
    arcpy.CreateFileGDB_management(Ruta_GDB_Salida, "Revision_Curvas", "CURRENT")
...
if __name__ == '__main__':
    ...
    Validacion_Dominios = sys.argv[3]
    Conteo_identificadores = sys.argv[4]
    Generacion_Marcos_Control = sys.argv[5]
    limpiar_Capas_Vacias =sys.argv[6]
    Marcos_De_Control_Orto_y_MDT = sys.argv[7]
    Revision_Curvas_Nivel_y_Drenajes = sys.argv[8]
    ...
    if Validacion_Dominios == 'true':
        ValidacionDominios(GDB, RutaSalida)
    if Conteo_identificadores  == 'true':
        ConteoIdentificadores(GDB, RutaSalida)
    if Generacion_Marcos_Control == 'true':
         GeneracionDeMarcos(GDB,RutaSalida, Escala, Limite_Del_Proyecto, Generacion_Marcos_Control)
    ...
    if Revision_Curvas_Nivel_y_Drenajes == 'true':
       RevisionCurvasNivelyDrenajes(RutaSalida)
